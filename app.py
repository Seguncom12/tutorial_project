...
def student_dashboard():
    if session.get('role') == 'student':
        return render_template('student_dash.html', announcements=ANNOUNCEMENTS, schedule=SCHEDULE, videos=VIDEO_LESSONS, live_link=LIVE_CLASS_LINK)
    return redirect(url_for('login'))

@app.route('/tutor_dashboard')
def tutor_dashboard():
    if session.get('role') == 'tutor':
        return render_template('tutor_dash.html', announcements=ANNOUNCEMENTS, schedule=SCHEDULE)
    return redirect(url_for('login'))

@app.route('/post_announcement', methods=['POST'])
def post_announcement():
    text = request.form.get('announcement')
    if text: ANNOUNCEMENTS.insert(0, text)
    return redirect(url_for('tutor_dashboard'))

@app.route('/add_schedule', methods=['POST'])
def add_schedule():
    SCHEDULE.append({"day": request.form.get('day'), "time": request.form.get('time'), "subject": request.form.get('subject')})
    return redirect(url_for('tutor_dashboard'))
...
@app.route('/join_live')
def join_live():
    if 'user' in session: return render_template('live_class.html', link=LIVE_CLASS_LINK)
    return redirect(url_for('login'))
